...
### E2 (major). The paper's formula for HINTS is wrong; the code follows the original HINTS convention, and that convention is what drives the headline effect

Section 3 defines HINTS by `S_t = 1[t mod τ > 0] + 1`, which with the paper's indexing (`u^{(t+1)} = u^{(t)} + C_{S_t}(...)`, `t ≥ 0`) would select the neural operator at `t = 0`. That is not what HINTS does and not what the code does. Algorithm S3 of Zhang et al. (arXiv 2208.13273, supplement) initialises `k_it ← 1` and applies the DeepONet when `k_it mod n_r = 0`, incrementing at the end of the loop, so the first call is at iteration `n_r` after `n_r − 1` relaxation steps; the HINTS authors' released implementation (`HINTS_precond/HINTS_numpy/iterative_solver.py`) uses `if (index + 1) % NUMERICAL_TO_DON_RATIO == 0` with `index` from 0; and this repository's `hybrid.py:246-247` (`(it + 1) % tau == 0`) and the old `HINTSRouter` (`hybrid_solver.py:55`) implement the same rule. The baseline is therefore faithful to HINTS, and the exposition must be corrected: either let `t` start at 1, or write `S_t = 1[(t + 1) mod τ > 0] + 1`. (The HINTS paper reports its schedule as a proportion `1/n_r` with `n_r = 25` for 1-D Poisson and `15` for the 2-D/3-D cases, which supports the paper's choice of τ = 25 as "the published period".)

The substantive point stands independently of whose convention is used. From `results/Poisson_128_jacobi.json`: the router reaches `h²` at iteration 2 (one corrector call, one sweep); HINTS-25 at iteration 25; HINTS-5 at iteration 5. Work-unit arithmetic with the paper's own costs reproduces the reported medians exactly (router 0.74 ms, HINTS-25 1.92 ms, HINTS-5 0.89 ms) and shows that the entire margin at `h²` is the τ−1 sweeps spent before the first call. With a corrector this strong, any practitioner would shift the phase and call it at `t = 0`; that schedule (HINTS's rule with the phase shifted, at any τ), or `τ ∈ {1, 2}`, is the fair fixed-schedule comparison and is absent; the "best τ" search covers only `{5, 10, 25, 50}`. The same phase effect produces the "orders of magnitude" AUC gap (HINTS-25 spends 24 iterations at relative error ≈ 1, so its AUC ≈ 24). The 1e-8 regime, where the router's advantage over the best period is 1.06× to 1.76× and comes from not re-calling the corrector when it no longer helps, is the part of the comparison that a phase-shifted HINTS does not erase.
...
| HINTS = `1[t mod τ > 0] + 1` (Sec. 3) | Formula wrong for 0-indexed `t`; code matches HINTS Alg. S3 and the HINTS authors' code (first NO call at iteration τ) | E2 |
...
Added here: the invalid "reverse triangle" step and the resulting sharper constant (T2); the vacuity of the bound relative to `ρ^{2T}` and the paper's own `α(O)` values (T3); the error in the paper's formula for HINTS, verified against the HINTS supplement and released code, and the phase-driven nature of the h² margin (E2); the quantitative implementation-efficiency confound with compiled measurements and its effect on the multigrid and best-τ claims (E3); the root cause of the SymGS mismatch in the dense code at HEAD (E4); the cost-equalisation numbers (T7); the final-error artefact of the stopping rule (E7); the std/s.e. labelling (E8); and the 512² statements in the abstract (E9).
