...
class PDE:
    ...
    def compute_residual(self, u_approx):
        ...
        if self.is_batch:
            output = torch.bmm(self.A, u_approx.unsqueeze(-1)).squeeze(-1)
        else:
...
class PoissonEquation2D(PDE):
    ...
    def solve(self):
        ...
        return u.squeeze(-1).reshape((self.batch_size, len(self.x), len(self.y))) if self.is_batch else u.squeeze(-1).reshape((len(self.x), len(self.y)))
...
class ReactionDiffusion2D(PDE):
    """
    Discretizes and solves:  -∇·(a∇u) + c*u = f  on a 2D uniform grid.
    With constant c > 0 the operator is positive-definite (no null space),
    so the system is uniquely solvable without a mean-zero constraint.

    Parameters
    ----------
    a_func : callable or tensor – diffusion coefficient.
    f_func : callable or tensor – RHS forcing, shape (batch, N*N) for batch.
    reaction : float – constant reaction coefficient c (must be > 0).
    boundary : {'Dirichlet', 'Periodic'}
    x, y : 1D tensors of grid points.
    """

    def __init__(self, a_func, f_func, reaction, boundary, x, y,
                 A=None, solve=True, device='cpu'):
        super().__init__(a_func, f_func, boundary, x, y, A)
        self.reaction = reaction
        self.is_batch = False
        self.batch_size = 1
    ...
    def build_matrix_periodic(self):
        ...
        for i in range(n):
            for j in range(m):
                ...
                self._set_matrix_entry(A, i, j, i, j, (a_imh + a_iph) / h_x**2 + (a_jmh + a_jph) / h_y**2 + c)
        return A
