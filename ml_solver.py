...
class MLSolver(torch.nn.Module):
    def __init__(self, dim , coef = False):
        super().__init__()
        self.dim = dim
        self.coef = coef
...
class DeepONet(MLSolver):
    ...
    def forward(self, input, coords = None):
        ...
        return out.reshape(-1, self.N, self.N) if self.dim ==2 else out.reshape(-1, self.N)

class FNOforPDE(MLSolver):
    def __init__(self, trunc_mode, dim, coef=False, hidden_size = 32, num_layers = 2):
        super().__init__(dim, coef)
        self.fno = FNO(n_modes = (trunc_mode,)*dim,
                       in_channels = 2 if coef else 1,
                       out_channels = 1,
