...
\noindent\textbf{Status of the confirmatory re-run (draft for review).} In response to five rounds of review, every wall-clock result is being regenerated under a revised, frozen protocol (\Cref{sec:wallclock_protocol}): compiled classical kernels, a corrected forcing sampler, per-operation costs measured in the form in which they are charged, the deployed rule equal to \Cref{alg:greedy} on macro-actions, HINTS with its two-dimensional period $\tau = 15$, residual-decay controls executed live, classical baselines timed in the same session as the policies with validated Krylov replays, a drift guard with re-timing, two-sided tests with bootstrap intervals and explicit censoring classes, a fresh test seed and three warmed-up timed replays. At the time of this draft the following are \emph{final}: the theory corrections (\Cref{sec:greedy,sec:approxgreedy} and their proofs); the wording of every claim is macro-driven and will be recomputed from the complete tables. The $128^2$ pairwise cells of Poisson and convection--diffusion were produced by an earlier revision of the driver (before the live decay rule, the Krylov validation, the stored decision sequences and the in-cell re-timing existed), and every one of their instances is re-timed with the final driver once the other $128^2$ cells are complete; those of anisotropic diffusion were produced by the final driver except for two details of the drift guard (it slept rather than ran the reference operation while waiting, and it compared an instance only with the reference at the time of its check), and the re-timing pass re-times any of their instances that were slow against the cell's final reference. Until the re-timing, the macros that depend on those fields (decision-sequence agreement, Krylov validity, re-timing counts) print as pending. Every $128^2$ pairwise cell and ensemble of the four equations is final (re-timing included), and every number computed from them is. Every Poisson and convection--diffusion cell at $128^2$, $256^2$ and $512^2$ is final as well, with their ensembles. Everything marked \textbf{[pending]} in the text or shown as a \textbf{[results pending]} table is still running, in the order of \texttt{run\_all.sh}: the variable-coefficient cells at $256^2$ with their ensembles and development-set selection, which also fix the grid-wide $256^2$ ranges. Everything else is final: every $128^2$ result, every Poisson, convection--diffusion and anisotropic cell at the larger grids with their ensembles, the development-set schedule selection at $128^2$ and $512^2$, the seed trials, usage traces and assumption and screening checks at every grid, the overheads and the discretisation study. Summary ranges quoted in the abstract and in \Cref{sec:experiments} are computed from the cells available at build time. Where a first confirmatory run (before the protocol revision) had produced a number that is quoted in the text, it is labelled as such.}
...
\begin{figure}[t]
    ...
    \caption{\edit{Corrector usage on the $\caN\times\caN$ grid: fraction of test runs that execute a corrector call at each iteration, for the learned router, the cost-aware oracle and HINTS ($\tau{=}15$), for every solver pairing (columns) and all four equations (rows); the same figure for the $256^2$ and $512^2$ grids is in \Cref{sec:wallclock}.}}
    \label{fig:ca_usage_main}
\end{figure}

\edit{\Cref{fig:ca_decisions_overview} shows the individual operation sequences of the learned router on every test instance, next to HINTS on the same instances. Every result per equation and grid, the ensemble study, the accounting of overheads and training cost, and the verification of the theoretical assumptions are collected in \Cref{sec:wallclock}; \Cref{sec:moreexperimentalresults} reports the Dirichlet experiment of the original submission.}
