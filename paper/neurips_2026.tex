...
\noindent\textbf{Status of the confirmatory re-run (draft for review).} In response to three rounds of review, every wall-clock result is being regenerated under a revised, frozen protocol (\Cref{sec:wallclock_protocol}): compiled classical kernels, a corrected forcing sampler, per-operation costs measured in the form in which they are charged, the deployed rule equal to \Cref{alg:greedy} on macro-actions, HINTS with its two-dimensional period $\tau = 15$, residual-decay controls, classical baselines timed in the same session as the policies, a drift guard, two-sided tests with bootstrap intervals, a fresh test seed and three warmed-up timed replays. At the time of this draft the following are \emph{final}: the theory corrections (\Cref{sec:greedy,sec:approxgreedy} and their proofs) and the wording of every claim, which is macro-driven and will be recomputed from the complete tables. Everything marked \textbf{[pending]} in the text or shown as a \textbf{[results pending]} table is still running, in this order: the $128^2$ pairwise cells with their baselines; the $128^2$ ensembles; all $256^2$ and $512^2$ cells; usage traces, seed trials, overheads, the assumption, screening and theorem checks and the discretisation study; variable-coefficient diffusion at $128^2$ and $256^2$ (\Cref{sec:wc_varcoeff}); and the decision-granularity ablation (\Cref{sec:wallclock_granularity}). Summary ranges quoted in the abstract and in \Cref{sec:experiments} are computed from the cells available at build time. Where a first confirmatory run (before the protocol revision) had produced a number that is quoted in the text, it is labelled as such.}
...
\begin{itemize}
    ...
    \item We empirically demonstrate that our approximate greedy solver achieves faster convergence than HINTS and individual solvers on the Poisson, convection-diffusion, anisotropic and variable-coefficient diffusion equations\edit{, in iterations and in wall-clock time on $128^2$ to $512^2$ grids with compiled classical kernels and with the classical baselines timed in the same session. At truncation-level accuracy the router takes the decisions of the best static schedule (one corrector call, then smoothing) on \caAgreeBestMin--\caAgreeBestMax\% of the instances without a search, and its paired time ratios are \caSpBestMin--\caSpBestMax\ against that schedule and \caSpHintsMin--\caSpHintsMax\ against HINTS with its published period; below truncation level the ratios are \caSpHintsDeepMin--\caSpHintsDeepMax\ against HINTS, \caSpOneshotDeepMin--\caSpOneshotDeepMax\ against the one-shot schedule and \caSpBestDeepMin--\caSpBestDeepMax\ against the best fixed schedule chosen on the test set (faster in \caCellsRouterBeatsBestDeep\ of \caNumCells\ pairings at $128^2$), while the ratio of multigrid alone to the router is \caVsMgDeepMin--\caVsMgDeepMax\ there. The construction extends to ensembles of classical solvers (\Cref{sec:wallclock_ens}; \textbf{[pending]})}.
\end{itemize}
...
\end{proposition}
The conditions of Proposition \ref{th:weaklyalphasupermodular} are natural. \edit{The corrector of \Cref{sec:experiments} leaves the modes outside its band untouched and has $\rho = 1$ exactly; the proposition permits this provided the optimal sequence contains at least one strictly contractive action in the same norm on the invariant subspace of the initial error (here the mean-free, Nyquist-free subspace, on which the smoothers are strict contractions), and for the exact common-unitary case Proposition~\ref{th:simultaneoussupermodular} applies without any strict contraction.} For classical solvers, the Lipschitz constant is $\|I_N - C_j \mathcal{L}^a_h\|$, which is typically less than $1$
