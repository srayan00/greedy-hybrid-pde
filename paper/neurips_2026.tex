...
\noindent\textbf{Status of the confirmatory re-run (draft for review).} In response to the two reviews, every wall-clock result is being regenerated under a frozen protocol (compiled classical kernels, a fresh test seed, three warmed-up timed replays, the complete fixed-schedule family; \Cref{sec:wallclock_protocol}). At the time of this draft the following are \emph{final}: the theory corrections (\Cref{sec:greedy,sec:approxgreedy} and their proofs), the exact short-horizon verification of \Cref{th:suboptgreedy} and the assumption tables of \Cref{sec:wallclock_assumptions} (provisional: computed in the development run with the numpy kernels; the operators are identical and only the macro-action sizes change), the discretisation-error study, the $128^2$ pairwise results for all three periodic equations (with the classical baselines for Poisson and convection--diffusion), and the development-versus-confirmatory comparison for those cells (\Cref{sec:wallclock_heldout}). Everything marked \textbf{[pending]} in the text or shown as a \textbf{[results pending]} table is still running, in this order: the nested and larger ensembles at $128^2$; all $256^2$ cells with their ensembles and baselines; all $512^2$ cells; usage traces, seed trials, overheads, and the confirmatory assumption and screening tables; variable-coefficient diffusion at $128^2$ and $256^2$ (\Cref{sec:wc_varcoeff}); and the decision-granularity ablation (\Cref{sec:wallclock_granularity}). Summary ranges quoted in the abstract and in \Cref{sec:experiments} are computed from the cells available at build time and will change as cells are added. Figures that depend on pending runs (usage traces on the larger grids, the variable-coefficient usage figure) show the development-run version or a placeholder.}
