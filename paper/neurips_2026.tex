...
\edit{\paragraph{Paired solvers.} \Cref{tab:ca_main} reports, for every pairing $\{\mathrm{NO}, \text{solver}\}$, equation and grid, the median wall-clock time to $\varepsilon = h^2$ of HINTS, of the best fixed schedule, and of the learned router, together with the classical methods without corrector. Two regimes appear. \emph{At truncation level} the whole solve is one corrector call followed by a few sweeps: the router, the cost-aware oracle and the static one-shot schedule all take the same decisions, so the router matches the best fixed schedule (paired ratios \caSpBestMin--\caSpBestMax\ at $128^2$, \caSpBestBMin--\caSpBestBMax\ at $256^2$, \caSpBestCMin--\caSpBestCMax\ at $512^2$, i.e., within the timer's resolution of parity and never significantly slower) and is \caSpHintsMin--\caSpHintsMax\ faster than HINTS with its published period $\tau{=}25$ (\caSpHintsBMin--\caSpHintsBMax\ at $256^2$, \caSpHintsCMin--\caSpHintsCMax\ at $512^2$), whose first call comes $\tau - 1$ sweeps too late; the classical solver alone is \caSpSolverMin--\caSpSolverMax\ slower on the isotropic equations and never reaches $h^2$ on the anisotropic one. \emph{Below truncation level} the schedule matters: down to $\varepsilon = 10^{-8}$ the router is \caSpBestDeepMin--\caSpBestDeepMax\ faster than the best fixed schedule at $128^2$ (\caSpBestDeepBMin--\caSpBestDeepBMax\ at $256^2$) and \caSpHintsDeepMin--\caSpHintsDeepMax\ faster than HINTS ($\tau{=}25$), because it calls the corrector only when the smooth error has re-emerged (two or three calls in total) whereas every fixed schedule either calls it far too often or, in the one-shot case, never again (\Cref{fig:ca_usage_main}); it tracks the cost-aware oracle within \caSpOracleRatioMax\ in median time and its decisions are identical across five independent training seeds. Against the classical methods without corrector the router with its best stationary pairing is \caVsMgMin--\caVsMgMax\ faster than geometric multigrid alone and \caVsKrylovMin--\caVsKrylovMax\ faster than multigrid-preconditioned CG or BiCGSTAB at $128^2$, and routing over $\{\mathrm{NO}, \text{multigrid}\}$ is \caVsMgEnsMin--\caVsMgEnsMax\ faster than multigrid alone; on the constant-coefficient equations the FFT direct solve remains \caFftRatioMin--\caFftRatioMax\ faster than any iterative method including ours, whereas on variable-coefficient diffusion, which has no such solve, the router is compared with the sparse direct solve (\Cref{sec:wc_varcoeff}). The iteration-based metrics tell the same story: at $128^2$ the router's AUC over $T = \caT$ iterations is orders of magnitude below HINTS' and the solver-only baseline's for every pairing and equation (paired $t$-tests, $p < 10^{-3}$; \Cref{tab:ca_auc_poisson,tab:ca_auc_conv,tab:ca_auc_aniso,tab:ca_auc_var}).}
