...
\begin{itemize}
    ...
    \item We evaluate the approximate greedy router against HINTS and individual solvers on the Poisson, convection-diffusion, anisotropic and variable-coefficient diffusion equations\edit{, in iterations and in wall-clock time on $128^2$ to $512^2$ grids with compiled classical kernels and with the classical baselines timed in the same session. At truncation-level accuracy the router executes the same decision sequence as the best static schedule (one corrector call, then smoothing) on \caAgreeBestMin--\caAgreeBestMax\% of the instances without a search, and its paired time ratios are \caSpBestMin--\caSpBestMax\ against that schedule and \caSpHintsMin--\caSpHintsMax\ against HINTS with its published period; below truncation level the ratios are \caSpHintsDeepMin--\caSpHintsDeepMax\ against HINTS, \caSpOneshotDeepMin--\caSpOneshotDeepMax\ against the one-shot schedule and \caSpBestDeepMin--\caSpBestDeepMax\ against the best fixed schedule chosen on the test set (faster in \caCellsRouterBeatsBestDeep\ of \caNumCells\ pairings at $128^2$), while the ratio of multigrid alone to the router is \caVsMgDeepMin--\caVsMgDeepMax\ there. The construction extends to ensembles of classical solvers (\Cref{sec:wallclock_ens}; \textbf{[pending]})}.
\end{itemize}
...
In \Cref{sec:approxgreedy}, we relax this assumption using a practical learning strategy that is Bayes consistent with this omniscient rule\edit{: for the full-information router, in the infinite-sample limit, it recovers the greedy decision at every visited state, which is a per-decision statement (\Cref{sec:approxgreedy} states what the feature-based router inherits); the sequential guarantee below is stated for the rule with access to the true error}.
...
\edit{Postfix monotonicity ($\mu = 1$) holds when the error propagation maps share a unitary eigenbasis---constant-coefficient periodic operators with Jacobi-type smoothers and band-limited spectral correctors---but it can fail for non-commuting contractions even when all of them are invertible, and a Gauss--Seidel sweep or a learned corrector need not be invertible at all. For the non-commuting solvers of our experiments no a priori bound on $\mu$ is available; \Cref{sec:wallclock_assumptions} computes $\mu$, $\alpha(O)$ and the bound of \Cref{th:suboptgreedy} by exhaustive enumeration for short horizons ($T \le 4$ macro-actions) on every ensemble of the study, including those with Gauss--Seidel, SOR and multigrid, and reports the largest $\mu$ (\caThmMuMax), the largest $\alpha(O)$ over greedy prefixes (\caThmAlphaGreedyMax) and the number of instance--ensemble pairs on which the theorem's premises were verified (\caThmPremises); at the horizons of tens to hundreds of macro-actions at which the oracle actually runs, neither $\alpha$ in the Euclidean norm nor $\mu$ is established for the non-normal sweeps, and no guarantee is claimed for them.} 
