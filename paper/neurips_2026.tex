...
When solving PDEs, classical numerical solvers are often computationally expensive, while machine learning methods can suffer from spectral bias, failing to capture high-frequency components. Designing an optimal hybrid iterative solver--where, at each iteration, a solver is selected from an ensemble of solvers to leverage their complementary strengths--poses a challenging combinatorial problem. While greedy selection is desirable for its constant-factor approximation guarantee to the optimal solution under Lipschitz \edit{and shared-eigenbasis} assumptions, it requires knowledge of the true error at each step, which is unavailable in practice. We address this by proposing an approximate greedy router that efficiently mimics a greedy approach to solver selection. Empirical results on the Poisson, convection–diffusion and anisotropic diffusion equations show that our method reduces the area-under-the-curve (AUC) of the error trajectory relative to single-solver baselines and existing hybrid approaches such as HINTS in every setting, reaching comparable error levels in substantially fewer iterations with more stable error decay. \edit{With a cost-aware instantiation of the greedy rule, a lightweight router, and a band-limited scale-equivariant corrector, we also measure end-to-end wall-clock time on $128^2$ to $512^2$ grids with compiled classical kernels, four equations (Poisson, convection--diffusion, anisotropic and variable-coefficient diffusion) and classical solvers from Jacobi to geometric multigrid, against HINTS, the best fixed schedule and the best residual-decay rule chosen per setting on the test set, and multigrid, preconditioned Krylov and direct solves timed in the same session. At truncation-level accuracy the learned router reproduces, without any search, the best static schedule found by a test-set search (one corrector call, then smoothing), to within \caSpBestMin--\caSpBestMax\ of it, and is \caSpHintsMin--\caSpHintsMax\ faster than HINTS with its published period; below truncation level it tracks the cost-aware oracle and beats HINTS and the one-shot schedule by large factors, but it is faster than the best fixed schedule in only \caCellsRouterBeatsBestDeep\ of \caNumCells\ pairings at $128^2$, and a compiled geometric multigrid remains the strongest classical method there; routing over ensembles of classical solvers is free at truncation level and, at the coarse decision granularity that cheap compiled sweeps impose, gains nothing over the best single solver at $128^2$ \textbf{[$256^2$, $512^2$ and finer-granularity cells pending]}.}
...
\begin{itemize}
    ...
    achieves a constant-factor approximation to the optimal strategy for linear PDEs, provided the updates are error-reducing (Lipschitz with constant $<1$) and zero-preserving--conditions under which weak supermodularity holds\edit{---and share a unitary eigenbasis, which gives the postfix monotonicity the guarantee also needs; for other ensembles the guarantee carries a measured slack}.
    ...
    \item We empirically demonstrate that our approximate greedy solver achieves faster convergence than HINTS and individual solvers on the Poisson, convection-diffusion, anisotropic and variable-coefficient diffusion equations\edit{, in iterations and in end-to-end wall-clock time on $128^2$ to $512^2$ grids with compiled classical kernels and with the classical baselines timed in the same session. At truncation-level accuracy the router reproduces the best static schedule (one corrector call, then smoothing) without a search and is \caSpHintsMin--\caSpHintsMax\ faster than HINTS with its published period; below truncation level it beats HINTS and the one-shot schedule by large factors, whereas against the best fixed schedule and a residual-decay rule, both selected on the test set, the outcome depends on the pairing (\caCellsRouterBeatsBestDeep\ of \caNumCells\ pairings at $128^2$), and geometric multigrid alone remains faster there. The construction extends to ensembles of classical solvers, where we find no gain at the decision granularity the compiled kernels impose}.
\end{itemize}
...
In \Cref{sec:approxgreedy}, we relax this assumption using a practical learning strategy that is Bayes consistent with this omniscient rule\edit{: in the infinite-sample limit it recovers the greedy decision at every state it is trained on, which is a per-decision statement; the sequential guarantee below is stated for the rule with access to the true error}.
...
The proof of \Cref{th:suboptgreedy} appears in \Cref{sec:proofsuboptgreedy}. As $T \to \infty$, the factor $1 - \phi_T(\alpha(O))$ decreases to $1 - e^{-1/\alpha(O)}$, so the worst-case performance of the greedy rule saturates rather than degrades with horizon\edit{, provided $\alpha(O)$ and $\mu$ stay bounded as $T$ grows (both are defined through the optimal sequence of length $T$ and can depend on it)}. \edit{The bound concerns the \emph{reduction} of the error over $T$ steps relative to the best sequence of $T$ steps: with $\alpha = \mu = 1$ it guarantees a $(1 - e^{-1})$ fraction of the optimal reduction, uniformly over ensembles, but it is not a convergence rate---a single convergent solver applied $T$ times already reduces the error by $\rho^{2T}$, which for large $T$ is far below the $e^{-1} g(\emptyset)$ term. In the settings of \Cref{sec:wallclock} the greedy sequence is in fact optimal or within a few percent of the exact optimum over short horizons on most instances (\Cref{sec:wallclock_assumptions}), much better than the worst case.} Additionally, larger $\alpha(O)$, which indicates higher reward for delayed inclusions, loosens the suboptimality bound.  
...
\end{proposition}
The conditions of Proposition \ref{th:weaklyalphasupermodular} are natural. \edit{The corrector of \Cref{sec:experiments} leaves the modes outside its band untouched and has $\rho = 1$ exactly; the proposition still applies because the smoother in $O$ is a strict contraction.} For classical solvers, the Lipschitz constant is $\|I_N - C_j \mathcal{L}^a_h\|$, which is typically less than $1$
...
\edit{Postfix monotonicity ($\mu = 1$) holds when the error propagation maps share a unitary eigenbasis---constant-coefficient periodic operators with Jacobi-type smoothers and band-limited spectral correctors---but it can fail for non-commuting contractions even when all of them are invertible, and a Gauss--Seidel sweep or a learned corrector need not be invertible at all. For the non-commuting solvers of our experiments no a priori bound on $\mu$ is available; \Cref{sec:wallclock_assumptions} computes $\mu$, $\alpha(O)$ and the bound of \Cref{th:suboptgreedy} by exhaustive enumeration for short horizons ($T \le 4$ macro-actions) and finds $\mu \le \caThmMuMax$ and $\alpha(O) = 1$ on every ensemble of the study, including those with Gauss--Seidel, SOR and multigrid; at the horizons of tens to hundreds of macro-actions at which the oracle actually runs, only the worst-case constant of Proposition~\ref{th:weaklyalphasupermodular} is available, and it is loose.} 
...
(a) Since $m_j(S') = \sum_{t = 1}^{|S'|} \1_{S_t' = j} = \sum_{t = 1}^{|S|} \1_{S_t' = j} + \sum_{t = |S|}^{|S'|} \1_{S_t' = j} = \sum_{t = 1}^{|S|} \1_{S_t = k} + \sum_{t = 1}^{|B|} 1_{B_t = j} = m_j(S) + m_j(B)$, (b) since $|\lambda_{ji}| \leq 1$ (as $\|I_N - C_j\mathcal{L}_h^a\| \le 1$ and $P$ is unitary) and $m_j(B) \geq 0$ for all $j \in [K]$
...
\edit{Throughout the proof $\Tilde{c}_j$ stands for the normalised weight $\hat c_j$ of the statement, so that $\sum_j \Tilde{c}_j = 1$ at every state considered; consequently $\Tilde{c}_j \le \bar E := 1$ for all $j$ and $\sum_k B_k = (K-1)\sum_j \Tilde{c}_j = (K-1) E_{\min}$ with $E_{\min} := 1$. With these two constants the argument below, written for general bounds $\bar E$ and $E_{\min}$, is unconditional.} For a given $a_h, f_h$, let $u_h$ be $\mathcal{G}_h\left(a_h, f_h\right)$ where $\mathcal{G}_h$ denotes the solution operator acting on the grid $G_h$. Furthermore, let's consider routers of the form
\begin{equation*}
