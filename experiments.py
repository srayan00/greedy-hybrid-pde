...
def test_model(model, dataloader, in_channels, dim, loss = ApproxGreedyRouterLoss()):
    model.eval()
    errors_greedy = ()
    loss_greedy = ()
    residuals = ()
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            if in_channels > 1:
                ...
                a = None
            pred = model(f = f, a = a, u0=None, return_dict = True, training=True, ground_truth = output.reshape(bs, -1))
...
def run(model_type, ml_model_type, n_test, dim, boundary, equation, ckp_dir, model_name, ml_model_name, data_dir, in_channels, device, extra = 1000):
    ...
    if model_type == "lstm":
        print(f"HERE")
        if dim == 1:
            router = LSTMGreedyRouter(None, ml_arguments["N"]*(in_channels + 1), arguments["hidden_dim"], arguments["num_layers"], 2, arguments["dropout"]).to(device)
        else:
            router = LSTMGreedyRouter(None, ml_arguments["N"]*ml_arguments["N"]*(in_channels + 1), arguments["hidden_dim"], arguments["num_layers"], 2, arguments["dropout"]).to(device)
    ...
    model.eval()
    loss = ApproxGreedyRouterLoss()
    errors_greedy, loss_greedy, residuals_greedy, mode_one_greedy, mode_five_greedy, mode_ten_greedy = test_model(model, test_loader, in_channels, dim,loss)
    ...
    errors_constant_jacobi, loss_constant_jacobi, residuals_constant_jacobi, mode_one_constant_jacobi, mode_five_constant_jacobi, mode_ten_constant_jacobi = test_model(model_constant_jacobi, test_loader, in_channels, dim, loss)
    ...
    errors_constant_gs, loss_constant_gs, residuals_constant_gs , mode_one_constant_gs, mode_five_constant_gs, mode_ten_constant_gs = test_model(model_constant_gs, test_loader, in_channels, dim, loss)
    ...
    errors_hints, loss_hints, residuals_hints , mode_one_hints, mode_five_hints, mode_ten_hints = test_model(model_hints, test_loader, in_channels, dim, loss)
    ...
    errors_hints_gs, loss_hints_gs, residuals_hints_gs ,mode_one_hints_gs, mode_five_hints_gs, mode_ten_hints_gs = test_model(model_hints_gs, test_loader, in_channels, dim, loss)
    ...
    if model_type == "lstm":
        print(f"HERE")
        if dim == 1:
            router_gs = LSTMGreedyRouter(None, ml_arguments["N"]*(in_channels + 1), arguments["hidden_dim"], arguments["num_layers"], 2, arguments["dropout"]).to(device)
        else:
            router_gs = LSTMGreedyRouter(None, ml_arguments["N"]*ml_arguments["N"]*(in_channels + 1), arguments["hidden_dim"], arguments["num_layers"], 2, arguments["dropout"]).to(device)
    ...
    errors_greedy_gs, loss_greedy_gs, residuals_greedy_gs, mode_one_greedy_gs, mode_five_greedy_gs, mode_ten_greedy_gs = test_model(model_greedy_gs, test_loader, in_channels, dim, loss)
    ...
    if model_type == "lstm":
        print(f"HERE")
        if dim == 1:
            router_gs_jac = LSTMGreedyRouter(None, ml_arguments["N"]*(in_channels + 1), arguments["hidden_dim"], arguments["num_layers"], 3, arguments["dropout"]).to(device)
        else:
            router_gs_jac = LSTMGreedyRouter(None, ml_arguments["N"]*ml_arguments["N"]*(in_channels + 1), arguments["hidden_dim"], arguments["num_layers"], 3, arguments["dropout"]).to(device)
    ckp = None
    ...
    errors_greedy_gs_jac, loss_greedy_gs_jac, residuals_greedy_gs_jac, mode_one_greedy_gs_jac, mode_five_greedy_gs_jac, mode_ten_greedy_gs_jac = test_model(model_greedy_gs_jac, test_loader, in_channels, dim, loss)
    ...
    if model_type == "lstm":
        print(f"HERE")
        if dim == 1:
            router_mg = LSTMGreedyRouter(None, ml_arguments["N"]*(in_channels + 1), arguments["hidden_dim"], arguments["num_layers"], 2, arguments["dropout"]).to(device)
        else:
            router_mg = LSTMGreedyRouter(None, ml_arguments["N"]*ml_arguments["N"]*(in_channels + 1), arguments["hidden_dim"], arguments["num_layers"], 2, arguments["dropout"]).to(device)
    ckp = None
    ...
    errors_greedy_mg, loss_greedy_mg, residuals_greedy_mg, mode_one_greedy_mg, mode_five_greedy_mg, mode_ten_greedy_mg = test_model(model_greedy_mg, test_loader, in_channels, dim, loss)
    ...
    errors_constant_mg, loss_constant_mg, residuals_constant_mg, mode_one_constant_mg, mode_five_constant_mg, mode_ten_constant_mg = test_model(model_constant_mg, test_loader, in_channels, dim, loss)
    ...
    errors_hints_mg, loss_hints_mg, residuals_hints_mg, mode_one_hints_mg, mode_five_hints_mg, mode_ten_hints_mg = test_model(model_hints_mg, test_loader, in_channels, dim, loss)
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            pred = model_hints(f = f, a = a, u0=None, return_dict = True, training=True, teacher_forcing = 1.0, ground_truth = output.reshape(bs, -1))
    ...
    print(df_error.to_latex(columns = ["FinalError_1d_Poisson", "AUC_Error_1d_Poisson", "FinalError_2d_Poisson", "AUC_Error_2d_Poisson", "FinalError_2d_Helmholtz", "AUC_Error_2d_Helmholtz"]))

    table = Texttable()
    table.add_row(["Router", "AUC", "Loss", "Empty"])
    table.add_row(["Jacobi Only", np.mean(auc_constant_jacobi)*(10**3), np.mean(loss_constant_jacobi).round(9), ""])
    table.add_row(["HINTS-Jacobi", np.mean(auc_hints)*(10**3), np.mean(loss_hints).round(9), ""])
    table.add_row(["Greedy-Jacobi", np.mean(auc_greedy)*(10**3), np.mean(loss_greedy).round(9), ""])
    table.add_row(["GS Only", np.mean(auc_constant_gs)*(10**3), np.mean(loss_constant_gs).round(9), ""])
    table.add_row(["HINTS-GS", np.mean(auc_hints_gs)*(10**3), np.mean(loss_hints_gs).round(9), ""])
    table.add_row(["Greedy-GS", np.mean(auc_greedy_gs)*(10**3), np.mean(loss_greedy_gs).round(9), ""])
    table.add_row(["Greedy-Jacobi,GS", np.mean(auc_greedy_gs_jac)*(10**3), np.mean(loss_greedy_gs_jac).round(9), ""])
    table.add_row(["MG Only", np.mean(auc_constant_mg)*(10**3), np.mean(loss_constant_mg).round(9), ""])
    table.add_row(["HINTS-MG", np.mean(auc_hints_mg)*(10**3), np.mean(loss_hints_mg).round(9), ""])
    table.add_row(["Greedy-MG", np.mean(auc_greedy_mg)*(10**3), np.mean(loss_greedy_mg).round(9), ""])

    table.add_row(["True Greedy", auc_true_greedy*(10**3), np.mean(loss_true_greedy).round(9), ""])
    print(f"Loss greedy {np.mean(loss_greedy).round(9)}")
    print(f"Loss greedy GS {np.mean(loss_greedy_gs).round(9)}")
    print(f"Loss greedy GS Jac {np.mean(loss_greedy_gs).round(9)}")
    print(f" Const {np.mean(loss_constant_jacobi).round(9)}")
    print(f"Hints loss {np.mean(loss_hints).round(9)}")
    print(f"Hints GS loss {np.mean(loss_hints_gs).round(9)}")
    print(f"true greedy {np.mean(loss_true_greedy).round(9)}")
    print(table.draw())
    ...
    table = Texttable()
    table.add_row(["Router", "AUC", "Loss", "Empty"])
    table.add_row(["Jacobi Only", f"{np.mean(auc_constant_jacobi*(10**3))} ({np.std(auc_constant_jacobi*(10**3))})","-", ""])
    table.add_row(["HINTS-Jacobi", f"{np.mean(auc_hints*(10**3))} ({np.std(auc_constant_jacobi*(10**3))})", "-", ""])
    table.add_row(["Greedy-Jacobi", f"{np.mean(auc_greedy*(10**3))} ({np.std(auc_greedy*(10**3))})", "", ""])
    table.add_row(["GS Only", f"{np.mean(auc_constant_gs*(10**3))} ({np.std(auc_constant_gs*(10**3))})", "", ""])
    table.add_row(["HINTS-GS", f"{np.mean(auc_hints_gs*(10**3))} ({np.std(auc_hints_gs*(10**3))})", "", ""])
    table.add_row(["Greedy-GS", f"{np.mean(auc_greedy_gs*(10**3))} ({np.std(auc_greedy_gs*(10**3))})", "", ""])
    table.add_row(["Greedy-Jacobi,GS", f"{np.mean(auc_greedy_gs_jac*(10**3))} ({np.std(auc_greedy_gs_jac*(10**3))})", "", ""])
    table.add_row(["MG Only", f"{np.mean(auc_constant_mg*(10**3))} ({np.std(auc_constant_mg*(10**3))})", "", ""])
    table.add_row(["HINTS-MG", f"{np.mean(auc_hints_mg*(10**3))} ({np.std(auc_hints_mg*(10**3))})", "", ""])
    table.add_row(["Greedy-MG", f"{np.mean(auc_greedy_mg*(10**3))} ({np.std(auc_greedy_mg*(10**3))})", "", ""])
    table.add_row(["True Greedy", auc_true_greedy*(10**3), np.mean(loss_true_greedy).round(9), ""])
    print(table.draw())
    ...
    print(df_residual.to_latex(columns = ["FinalResidual_1d_Poisson", "AUC_Residual_1d_Poisson", "FinalResidual_2d_Poisson", "AUC_Residual_2d_Poisson", "FinalResidual_2d_Helmholtz", "AUC_Residual_2d_Helmholtz"]))

    table = Texttable()
    table.add_row(["Router", "AUC", "Loss", "Empty"])
    table.add_row(["Jacobi Only", f"{np.mean(auc_constant_jacobi*(10**3))} ({np.std(auc_constant_jacobi*(10**3))})", "", ""])
    table.add_row(["HINTS-Jacobi", f"{np.mean(auc_hints*(10**3))} ({np.std(auc_hints*(10**3))})", "", ""])
    table.add_row(["Greedy-Jacobi", f"{np.mean(auc_greedy*(10**3))} ({np.std(auc_greedy*(10**3))})", "", ""])
    table.add_row(["GS Only", f"{np.mean(auc_constant_gs*(10**3))} ({np.std(auc_constant_gs*(10**3))})", "", ""])
    table.add_row(["HINTS-GS", f"{np.mean(auc_hints_gs*(10**3))} ({np.std(auc_hints_gs*(10**3))})","", ""])
    table.add_row(["Greedy-GS", f"{np.mean(auc_greedy_gs*(10**3))} ({np.std(auc_greedy_gs*(10**3))})", "", ""])
    ...
    # table = Texttable()
    # table.add_row(["Router", "Mode 1 AUC", "Mode 5 AUC", "Mode 10 AUC"])
    # table.add_row(["Jacobi Only", f"{np.mean(modeoneauc_constant_jacobi*(10**3))}", modefiveauc_constant_jacobi*(10**3),modetenauc_constant_jacobi*(10**3)])
    # table.add_row(["HINTS-Jacobi", f"{np.mean(modeoneauc_hints*(10**3))}", modefiveauc_hints*(10**3), modetenauc_hints*(10**3)])
    # table.add_row(["Greedy-Jacobi", f"{np.mean(modeoneauc_greedy*(10**3))}", modefiveauc_greedy*(10**3), modetenauc_greedy*(10**3)])
    # table.add_row(["GS Only", f"{np.mean(modeoneauc_constant_gs*(10**3))}", modefiveauc_constant_gs*(10**3), modetenauc_constant_gs*(10**3)])
    # table.add_row(["HINTS-GS", f"{np.mean(modeoneauc_hints_gs*(10**3))}", modefiveauc_hints_gs*(10**3), modetenauc_hints_gs*(10**3)])
    # table.add_row(["Greedy-GS", f"{np.mean(modeoneauc_greedy_gs*(10**3))}", modefiveauc_greedy_gs*(10**3), modetenauc_greedy_gs*(10**3)])
    # table.add_row(["Greedy-Jacobi,GS", f"{np.mean(modeoneauc_greedy_gs_jac*(10**3))}", modefiveauc_greedy_gs_jac*(10**3), modetenauc_greedy_gs_jac*(10**3)])
    # table.add_row(["MG Only", f"{np.mean(modeoneauc_constant_mg*(10**3))}",  modefiveauc_constant_mg*(10**3),  modetenauc_constant_mg*(10**3)])
    # table.add_row(["HINTS-MG", f"{np.mean(modeoneauc_hints_mg*(10**3))}", modefiveauc_hints_mg*(10**3), modetenauc_hints_mg*(10**3)])
    # table.add_row(["Greedy-MG", f"{np.mean(modeoneauc_greedy_mg*(10**3))}", modefiveauc_greedy_mg*(10**3), modetenauc_greedy_mg*(10**3)])
