...
for EQ in Poisson ConvDiff AnisoDiff VarCoeff; do
  ...
  if [ -n "$TODO" ]; then
    [ -f checkpoints/router_${EQ}_128_${S##*,}_u4.pth ] && [ checkpoints/router_${EQ}_128_${S##*,}_u4.pth -nt libstencil.so ] || run $PY bench.py --equation $EQ --N 128 --solvers $S --train_only --retrain_router --unit_frac 0.25 --router_tag _u4 > logs/final_routers_${EQ}_128_u4.log 2>&1
    run $PY bench.py --equation $EQ --N 128 --solvers $TODO --n_test 64 --max_ops 60000 --unit_frac 0.25 --router_tag _u4 --tag _u4 --baselines none --policies $POL --seed 73 --timed_reps 3 > logs/final_bench_${EQ}_128_u4.log 2>&1
    new=1
  fi
  if [ ! -f results/${EQ}_128_ens_jacobi+jacobi_0.67_u4.json ]; then
  ...
  if [ $new = 1 ]; then stage "granularity $EQ done"; fi
done
