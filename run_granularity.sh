...
for EQ in Poisson ConvDiff; do for S in $(cat config/solvers_$EQ | tr , ' '); do
  run $PY bench.py --equation $EQ --N 128 --solvers $S --n_test 64 --max_ops 60000 --policies $POLF --seed 73 --timed_reps 3 --retime_only > logs/final_retime_${EQ}_128_$S.log 2>&1
done; done
